...
- **추가 설치 없음**: setup.bat 실행만으로 자동 활성화
- **추가 API 키 없음**: BM25 키워드 검색으로 동작
- **동작 방식**: 발언 직전 회의 주제 + 최근 대화 흐름으로 각 멤버의 관련 과거 발언 top-5 검색 → 에이전트 컨텍스트에 삽입

`team-skills/{slug}/slack_messages.json`이 있는 멤버만 검색이 활성화됩니다.
파일이 없는 멤버는 검색 없이 SKILL.md 페르소나만으로 발언합니다.
