...
def test_get_history_invalid_session_id_returns_400():
    ...
    httpx(TestClient 내부)는 '../../etc/passwd'를 '/etc/passwd'로 정규화하므로
    인코딩된 형태(%2e%2e%2f)를 사용한다 — 이 형태는 핸들러까지 도달한다.
    """
    res = _client().get("/api/history/..%2F..%2Fetc%2Fpasswd")
    assert res.status_code == 400


def test_delete_history_invalid_session_id_returns_400():
    res = _client().delete("/api/history/..%2F..%2Fetc%2Fpasswd")
    assert res.status_code == 400
